...
if (isset($_POST['tambah_alumni'])) {
    ...
    @mysqli_query($koneksi, "INSERT INTO alumni (nama, tahun_lulus, kesan) VALUES ('$nama', '$tahun', '$kesan')");
                ...
                ?>
                    ...
                    <form action="" method="POST" class="mb-4 p-3 border rounded bg-light">
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-md-8"><input type="text" name="nama" class="form-control" placeholder="Nama Alumni" required></div>
                            <div class="col-md-4"><input type="number" name="tahun_lulus" class="form-control" placeholder="Tahun Lulus" required></div>
                        </div>
                        <div class="mb-2"><textarea name="kesan" class="form-control" rows="2" placeholder="Kesan / Pesan Alumni"></textarea></div>
                        <button type="submit" name="tambah_alumni" class="btn btn-success btn-sm fw-bold"><i class="bi bi-plus-circle me-1"></i> Tambah Alumni</button>
                    </form>
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle">
                            <thead class="table-dark">
                                <tr><th>No</th><th>Nama</th><th>Tahun Lulus</th><th>Aksi</th></tr>
                            </thead>
                            <tbody>
                                <?php 
                                $no=1; 
                                $q=mysqli_query($koneksi, "SELECT * FROM alumni ORDER BY id DESC");
                                while($d=mysqli_fetch_assoc($q)): 
                                    $thn = $d['tahun_lulus'] ?? $d['tahun'] ?? '-';
                                ?>
                                <tr>
                                    <td><?= $no++; ?></td>
                                    <td><?= htmlspecialchars($d['nama'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($thn); ?></td>
                                    <td><a href="admin.php?tab=alumni&hapus=<?= $d['id']; ?>&from=alumni" class="btn btn-danger btn-sm" onclick="return confirm('Hapus data alumni ini?')"><i class="bi bi-trash"></i></a></td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
                        </table>
                ...
                ?>
                    ...
                    <form action="" method="POST" class="mb-4 p-3 border rounded bg-light">
                        ...
                        <div class="row g-2 mb-2">
                            ...
                            <!-- Opsi level langsung diset 'admin' tanpa pilihan superadmin -->
                            <input type="hidden" name="level" value="admin">
                            
                            <div class="col-md-2">
                                <button type="submit" name="tambah_user" class="btn btn-success w-100 fw-bold">
                        ...
                        <table class="table table-bordered align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge bg-secondary"><?= htmlspecialchars($lvl); ?></span></td>
                                    <td>
                                        <?php if(strtolower($d['username'] ?? '') !== 'm4570ko'): ?>
                                            
                                            <!-- Tombol Reset Password (Khusus M4570KO) -->
                                            <?php if ($is_m4570ko): ?>
                                                <button type="button" class="btn btn-warning btn-sm me-1" data-bs-toggle="modal" data-bs-target="#modalReset<?= $d['id']; ?>">
                                                    <i class="bi bi-key me-1"></i> Reset Password
                                                </button>

                                                <!-- Modal Form Reset Password -->
                                                <div class="modal fade" id="modalReset<?= $d['id']; ?>" tabindex="-1">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <form action="" method="POST">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title">Reset Password - <?= htmlspecialchars($d['username']); ?></h5>
                                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                                </div>
                                                                <div class="modal-body text-start">
                                                                    <input type="hidden" name="id_user" value="<?= $d['id']; ?>">
                                                                    <div class="mb-3">
                                                                        <label class="form-label fw-semibold">Masukkan Password Baru</label>
                                                                        <input type="password" name="password_baru" class="form-control" required placeholder="Password baru...">
                                                                    </div>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
                                                                    <button type="submit" name="reset_password" class="btn btn-success btn-sm">Simpan Password Baru</button>
                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
                                            ...
                                            <a href="admin.php?tab=users&hapus=<?= $d['id']; ?>&from=users" class="btn btn-danger btn-sm" onclick="return confirm('Hapus user ini?')"><i class="bi bi-trash"></i> Hapus</a>
