...
.news-slider-container, .alumni-slider-container {
    position: relative;
    overflow: hidden;
    padding: 10px 0;
}

.news-slider-track, .alumni-slider-track {
    display: flex;
    gap: 16px;
    overflow-x: auto;
...
.news-slider-track::-webkit-scrollbar, .alumni-slider-track::-webkit-scrollbar {
    display: none;
}

.news-card-item, .alumni-card-item {
    flex: 0 0 calc(25% - 12px); /* Tampil Presisi 4 Kolom dalam 1 Baris */
    min-width: 250px;
...
@media (max-width: 992px) {
    .news-card-item, .alumni-card-item {
        flex: 0 0 calc(50% - 10px); /* 2 Kolom untuk Tablet */
    }
}

@media (max-width: 576px) {
    .news-card-item, .alumni-card-item {
        flex: 0 0 100%; /* 1 Kolom untuk HP */
    }
}
        ...
        <div class="alumni-slider-track" id="alumniTrack">
            ...
            <div class="alumni-card-item">
                <div class="card h-100 shadow-sm border-0 text-center p-3">
                    <div class="mb-2">
                        <img src="uploads/<?= $alumni['foto']; ?>" class="img-alumni shadow-sm" alt="<?= $alumni['nama']; ?>">
                    </div>
                    <h6 class="fw-bold text-dark mb-1" style="font-size: 14px;"><?= $alumni['nama']; ?></h6>
                    <span class="badge bg-success-subtle text-success mb-1" style="font-size: 10px;"><?= $alumni['pendidikan']; ?></span>
                    <p class="text-muted small mb-2" style="font-size: 11px;"><i class="bi bi-geo-alt"></i> <?= $alumni['alamat']; ?></p>
                    <p class="card-text text-secondary small" style="font-size: 12px; line-height: 1.4;">
                        <?= $alumni['kesan_pesan']; ?>
                    </p>
                </div>
            </div>
            ...
            ?>
                <p class="text-muted small ms-2">Belum ada kesan & pesan alumni yang ditambahkan.</p>
            <?php endif; ?>
        </div>
    </div>
...
function scrollNews(direction) {
    if (newsTrack) {
        ...
        newsTrack.scrollBy({ left: direction * width, behavior: 'smooth' });
    }
}
...
if(newsTrack) {
    ...
    newsTrack.addEventListener('mouseleave', () => autoNews = setInterval(() => {
        if (Math.ceil(newsTrack.scrollLeft + newsTrack.clientWidth) >= newsTrack.scrollWidth - 5) { newsTrack.scrollTo({ left: 0, behavior: 'smooth' }); } else { scrollNews(1); }
    }, 3000));
}
...
if(alumniTrack) {
    ...
    alumniTrack.addEventListener('mouseleave', () => autoAlumni = setInterval(() => {
        if (Math.ceil(alumniTrack.scrollLeft + alumniTrack.clientWidth) >= alumniTrack.scrollWidth - 5) { alumniTrack.scrollTo({ left: 0, behavior: 'smooth' }); } else { scrollAlumni(1); }
    }, 2000));
}
</script>
